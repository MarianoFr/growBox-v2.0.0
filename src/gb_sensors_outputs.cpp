...
/***********************************************************************/
// Depending on the control variable's values, retrieved from DB,
...
void analogSoilRead(struct readControl *rx, struct writeControl *tx)
{
  ...
  if(((rgb_state >> BH_1750_ERR) & 1U) && ((rgb_state >> HTU21_ERR) & 1U))//sensor disconnected
  {
    return;
  }
